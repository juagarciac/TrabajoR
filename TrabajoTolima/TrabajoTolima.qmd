...
Oficiales de Colombia: Se priorizarán bases de datos y publicaciones de entidades gubernamentales como el Departamento Administrativo Nacional de Estadística (DANE), el Departamento Nacional de Planeación (DNP), el Departamento Nacional de Estadística (DANE), la Contaduría General de la Nación (CGN), y otras instituciones relevantes que publiquen información a nivel municipal.
...
Lenguaje de Programación: R (con paquetes como tidyverse, sf, ggplot2, leaflet, rmarkdown, knitr para la manipulación, análisis y visualización de datos).
...
En este apartado no analizaremos todas las variables, solo las que puedan tener mas relevancia para entender el estado fiscal del Tolima, pero antes que ello es necesario revisar los valores atípicos o valores que puedan entorpecer el análisis.
...
Al ver si hay valores faltantes se evidencio que no existen en el dataframe, pero si existen valores atípicos en varias variables, la cual es “ND”, la cual es el acrónimo de No Disponible o No Declarado, en este caso se manejara dependiendo de la situación a analizar, ya que no puede ser reconocida como 0 u otro valor numérico, pero en general se eliminara para poder realizar operaciones con variables puramente enteras o decimales, aunque puede darse que sea necesario tener este valor en cuenta.
...
# Analsis descriptivo para la base de datos ICFES
...
**Análisis de correlación para las variables númericas**
...
**Vizualización de Colombia**

```{r}
# Verificar la estructura (opcional)
# print(mapa_mpios)

# Graficar el shapefile
ggplot(data = mapa_mpios) +
  geom_sf(fill = "lightblue", color = "grey40") +
  theme_minimal() +
  labs(title = "Mapa de Municipios (MGN 2024)", caption = "Fuente: DANE")

```

**Vizualización del Tolima**
...
## Mapa de las distacias del municipio a la capital
...
# Analsis descriptivo índice de Pobreza Multidimencional
...
A nivel nacional, si hacemos un comparativo entre 2018 y 2024, encontramos que, en terminos generales, hay una tendencia de disminución de la pobreza multidimensional en casi todo el país, lo que demuestra que hay avances importantes en acceso a servicios básicos, educación, vivienda, entre otras.
...
Por el otro lado, los departamentos como Bogotá, San Andrés, Cundinamarca, Santander y Valle del Cauca son los que mas bajos niveles de IPM mantienen, con menos del 10% de la población en situación de pobreza multidimensional en 2024.
...
En el mapa anterior, se puede observar claramente la distribución territorial de la pobreza multidimensional y su evolución entre 2018 y 2024. Se ve que gran parte de la periféria del país, especialmente la Amazonía y la Orinoquía, junto con departamentos de la región Caribe y Pacífica, son los que más altos niveles de IPM muestran, llegando a superar el 60% en algunos casos y la zona centrica, por el otro lado, concentrando los niveles más bajos. Para el 2024, se evidencia una mejoría en las zonas periféricas, reducciendo el porcentaje de la población en situación de pobreza multidimensional en 22 de los 32 departamentos.
...
Se puede ver que son las zonas del sur y suroccidente del departamento las que concentrar los valores más altos de IPM, especialmente los municipios de Rioblanco, Ataco y Coyaima que presentan índices superiores al 60% y al promedio del departamento. En contraste, algunos municipios del norte y centro muestran menores niveles.

Por otro lado, al analizar algunas dimenciones por separado observamos lo siguiente:
